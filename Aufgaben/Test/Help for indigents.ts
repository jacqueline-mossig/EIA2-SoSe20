namespace L02_HelpForIndigents {
...
function handleLoad(_event: Event): void {                  //Funktion handleLoad erstellen
    ...
    let form: HTMLDivElement = <HTMLDivElement>document.querySelector("div#form");                          //HTML-Element #selectDefined aus HTML-Dokument holen, um den Listener installieren zu können
    form.addEventListener("change", handleChange);                                                          //Click-Listener installieren um choosenSequence aufzurufen
   
    let slider: HTMLInputElement = <HTMLInputElement>document.querySelector("input#timeRequired");                      //HTML-Element #inputOwn aus HTML-Dokument holen, um den Listener installieren zu können
    slider.addEventListener("input", displayAmount);                                                        //Click-Listener installieren um createSequence aufzurufen  

    let choose: HTMLFieldSetElement = <HTMLFieldSetElement>document.querySelector("fieldset#choose");
    choose.addEventListener("click", showInput);

    let okay: HTMLButtonElement = <HTMLButtonElement>document.querySelector("button#okay");
